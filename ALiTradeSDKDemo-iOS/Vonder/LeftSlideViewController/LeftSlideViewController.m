...
-(void)handeTap:(UITapGestureRecognizer *)tap{
    ...
    if ((!self.closed) && (tap.state == UIGestureRecognizerStateEnded))
    {
        ...
        if ([self.delegate respondsToSelector:@selector(didLeftViewClose)]) {
            [self.delegate didLeftViewClose];
        }
    }
...
- (void)closeLeftView
{
    ...
    self.mainVC.view.center = CGPointMake(kScreenWidth / 2, self.view.frame.size.height/2);
    self.closed = YES;
...
//    self.mainVC.view.center = kMainPageCenter;
    self.mainVC.view.center = CGPointMake(kScreenWidth + kScreenWidth * kMainPageScale / 2.0 - kMainPageDistance, self.view.frame.size.height / 2);
    self.closed = NO;
    ...
    if ([self.delegate respondsToSelector:@selector(didLeftViewOpen)]) {
        [self.delegate didLeftViewOpen];
    }
}
