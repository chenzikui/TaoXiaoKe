...
-(NSMutableString*)formatNameLabelWidth:(CGFloat)lb_width firstLineHeadIndent:(CGFloat)indent font:(UIFont*)font;{
    NSMutableString *name = [NSMutableString stringWithString:VALIDATE_STRING(self)];
    NSString *temp = nil;
    for(int i =0; i < [self length]; i++)
    {
        temp = [self substringWithRange:NSMakeRange(0, i)];
        CGFloat width = [temp textSizeWithFont:font maxSize:CGSizeMake(999, 999)].width;
        if (width > lb_width - indent && i>0) {
            NSMutableString *kong = [NSMutableString string];
            CGFloat x = 0;
            do
            {
                [kong appendString:@" "];
                x += [@" " textSizeWithFont:font maxSize:CGSizeMake(999, 999)].width;
            }while(x < indent);
            [name insertString:kong atIndex:i-1];
            break;
        }
    }
    return name;
